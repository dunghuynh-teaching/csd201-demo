...
public class Lib {
    ...
    // Read line k (k = 0,1,2,...) and extracts it to the array strings 
    ...
    static public String[] readLineToStrArray(String fname, int k) {
        ...
        try {
            ...
            for (int i = 0; i < k; i++) {
                f.readLine(); // skip n - 1 lines
            }
            s = f.readLine(); // read data at the line k^th
    ...
    //Read all lines in Input i of Function x and return String array 
    //Then loadData use regex to parse data
    static public String[] readAllLineInputOfFunction(String filename, int function, int input) {

        String functionTag = "FUNCTION " + function;
        String inputTag = "INPUT " + input;
        ...
        try {
            ...
            while (inputReader.hasNextLine()) {
                ...
                if (data.trim().toUpperCase().startsWith("#" + functionTag)) {
                    startFunction = true;
                } else if (startFunction && data.trim().toUpperCase().startsWith("#" + inputTag)) {
                    startInput = true;
                } else if (startInput && data.trim().toUpperCase().startsWith("#END " + inputTag)) {
                    break;
                } else if (startFunction && data.trim().toUpperCase().startsWith("#END " + functionTag)) {
                    break;
                } else if (startFunction && startInput) {
                    results.add(data);
                    i++;
                }
            ...
            System.out.println("Error at readAllLineInputOfFunction(). Error: " + e.getMessage());
            e.printStackTrace();
        }
        return null;
    }
